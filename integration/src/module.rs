...
/// A top level expression inside a module
#[derive(Clone)]
...
impl TopLevelExpression {
    /// Creates a new `TopLevelExpression` instance.
    /// 
    /// # Parameters
    ///
    /// - `ast`: An `AST` instance associated with this top level expression.
    /// - `sym_table_stack`: A `SymbolTableStack` instance associated with this top level expression's AST.
    /// 
    /// # Returns
    ///
    /// Returns a new `TopLevelExpression` instance with the given AST and STS associated with it.
    ///
    pub fn new(ast: AST, sym_table_stack: SymbolTableStack) -> Self {
        Self {
            ast,
            sym_table_stack,
        }
    ...
    /// Retrieves the AST associated with this `TopLevelExpression`
    /// 
    /// # Returns
    ///
    /// Returns a clone of the given `TopLevelExpression`'s `AST`.
    ///
    pub fn get_ast(&self) -> AST {
        self.ast.clone()
    }

    /// Retrieves the STS associated with this `TopLevelExpression`
    /// 
    /// # Returns
    ///
    /// Returns a clone of the given `TopLevelExpression`'s `SymbolTableStack`.
    ///
    pub fn get_sym_table_stack(&self) -> SymbolTableStack {
        self.sym_table_stack.clone()
    }
